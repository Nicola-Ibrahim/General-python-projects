...
class Topsis:
    ...
    def step_2(self):
        ...
        self.normalized_decision_matrix = self.normalized_decision_matrix / (
            sqrd_sum**0.5
        )
    ...
    def step_3(self):
        ...
        self.weighted_normalized_decision_matrix = (
            self.normalized_decision_matrix.copy()
        )

        self.weighted_normalized_decision_matrix = (
            self.weight_matrix * self.weighted_normalized_decision_matrix
        )
        ...
        for i in range(self.column_size):
            if self.criteria[i]:
                self.worst_alternatives[i] = min(
                    self.weighted_normalized_decision_matrix[:, i]
                )
    ...
    def step_5(self):
        ...
        self.worst_distance = (
            (self.worst_distance_mat - self.worst_alternatives) ** 2
        ).sum(axis=1) ** 0.5
        self.best_distance = (
            (self.best_distance_mat - self.best_alternatives) ** 2
        ).sum(axis=1) ** 0.5
    ...
    def get_rank(self):
        ...
        scores.insert(0, "rank", range(0, len(self.worst_similarity)))
